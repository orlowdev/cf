...
gate, specialize (type arguments and `[n 'T]` sizes), the memory arc (escape class
and node-free-ness — "returns only scalars"), and emit (`match` dispatch). Nothing
it annotates may be a runtime transformation; it only decorates and rejects.
...
**wraps** — two's-complement modular arithmetic, the hardware default, silent (§5.6).
A *comptime-known* overflow is a **compile error** (the compiler catches the obvious
bug); a runtime overflow wraps, and the std offers `wrapping_*`/`saturating_*`/
`checked_*` for when the developer wants a chosen or guaranteed result instead.
...
- **Full IEEE semantics — `NaN` and `±Infinity` are ordinary values.** A float
  ...
  code that needs the guarantee (§6/§9 cover its representation).
...
- **`Str`** — a string. Its representation (a static literal lives in read-only
  data and is not residue; a runtime-constructed string is an aggregate) is fixed
  by [[memory_model.md]]; the exact header layout is the aggregate-representation
  question (§9, and the M6/M9 gate). A string literal has type `Str` (§3).
...
- **`Never`** — the **bottom type**: the type of an expression that never yields a
  value (`break`, `continue`, a diverging call, an unbounded loop). `Never` is a
  subtype of every type, so a `Never`-typed expression is well-typed in any position
  (notably as an `if` branch). Nothing has a `Never` value at runtime; it types
  control that leaves rather than returns. This is the real "produces no value"
  type; `Unit`/`()` by contrast produces the one trivial value and returns normally.
...
- **No expected type is an error.** When nothing supplies a numeric expected type —
  ...
  context — the literal is rejected: it must be annotated (`const Int x = 5`) or
...
converting an already-typed `Int` value to a `Uint8` does.
...
developer's responsibility" rule — with a *comptime-known* out-of-range value a
**compile error**. Its guaranteed counterpart is an ordinary module function
...
A call may give type arguments explicitly (`max[Int](a, b)`) or leave them to be
...
- **integer `/ 0` and `% 0` are `0`** — a defined result on every target (the
  ...
  instruction already yields 0). Baffling but total.
- **float operations follow IEEE-754** — producing `NaN` / `±Infinity` as values
  (§2.2), never a crash.

The **compiler still rejects a comptime-known** overflow or `/0` — the obvious bug
is caught, not silently wrapped — but a runtime one is defined, not fatal. When the
developer wants a *chosen* or *guaranteed* result instead of the default, the std
supplies it, never the core operators: `wrapping_*` / `saturating_*` for explicit
...
Point(1, 2)              # a record from its fields, as an argument tuple
Maybe.Just(1)            # a union member from its payload tuple
Maybe.Nothing            # a nullary member — payload (), no application needed
```
...
- **Ascription is construction.** `const T x = v` is sugar for `const x = T(v)`
  ...
  `const x = Uint8(0)` are the same. The **prefix record literal `Point { … }` is
  dropped**; a record is built by `Point(…)` (or the annotated sugar), keeping
  construction symmetric with the `Point(x, y)` destructuring pattern.
...
the field *default* in the declaration, `Int x = 0`, deliberately distinct). Either
...
inside the union (compose-over). The numeric and boolean unions use the *latter* —
every width (`Int8`, `Int32`, `Arch`, …) and both boolean singletons (`False`,
`True`) is a **standalone, directly-usable type** (`const Int32 x = …`, `const b =
Bool.False`), and the unions merely **compose over** those existing types. So the
concrete types are first-class on their own; the unions add the "any of them" bound
on top without owning them:
...
1. **Tuples move from brackets to parentheses.** `[T1, …, Tn]` → `(T1, …, Tn)`;
   ...
   arm; `[…]` becomes arrays only (`array`, `fixed_array`). The **pattern** grammar
   follows: a `(…)` destructuring pattern (tuples/positional records) forks from the
   `[…]` `array_pattern` (arrays) — `pattern`/`array_pattern`/`destructure_decl` gain
   the paren form. (§6.1, §6.2, §7.1)
...
5. **Construction is application `T(...)`.** Add `type_name` as a callee (a
   ...
   (parens). (§6.5, §6.6)
6. **Drop the prefix record literal `Point { … }`** and the union-member
   juxtaposition `Member { … }`; records and members are built by `T(…)` (or the
   annotated sugar `const T x = …`). The bare brace literal in value position no
   longer means "typed record". (§6.6)
...
8. **The cast is `T(x)`** (no new `as`/intrinsic form); narrowing truncates
   (comptime-known out-of-range is a compile error), the `checked`-style guaranteed
   conversion is a module function returning `Either`. (§4)
9. **`Maybe`/`Either` replace `Option`/`Result`:** `Option`→`Maybe`, `Some`→`Just`,
   `None`→`Nothing`; add `Either`/`Left`/`Right`. The else-less-`if` rule yields a
   `Maybe`. These are library unions; the grammar references to `Option`/`Some`/
   `None` are renamed. (§3/§5.3; details in §8)
...
13. **Union bodies gain member spread.** `union Number = { ...Int, ...Uint, ...Float }`
    ...
    form (today `union_member` has no spread). (§8.2)
